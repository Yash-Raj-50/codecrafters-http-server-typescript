...
function createResponseHTTP1_1Func(requestData: sharedRequestParsedDataInterface): sharedResponseInterface {
    ...
    if (requestData.method === "GET") {
        if (requestData.target === '/') {
            ...
            response.headers["Content-Length"] = response.body.toString().length;
            return response;
            ...
            response.headers["Content-Length"] = response.body.toString().length;
            return response;
        } else if (requestData.target.startsWith('/user-agent')) {
            response.statusLine.statusCode = 200;
            response.statusLine.statusMessage = "OK";
            response.body = requestData.headers["user-agent"];
            response.headers["Content-Length"] = response.body.toString().length;
            return response;
        } else {
